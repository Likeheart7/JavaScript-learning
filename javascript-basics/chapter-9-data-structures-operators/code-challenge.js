...
6. Write a function ('printGoals') that receives an arbitrary number of player names (NOT an array) and prints each of them to the console, along with the number of goals 
...
const printGoals = (...names) => {
  console.log(`${names} goals were scored`);
};
...
const sum = val.reduce((acc, cur) => acc + cur);
...
for (const [index, statement] of list.entries()) {
  const words = statement.toLowerCase().split('_');
  const result =
